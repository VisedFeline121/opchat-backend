...
class TestChatFlow:
    ...
    def test_complete_dm_creation_flow(
        self, client: TestClient, user_repo, clean_db, placeholder_user
    ):
        ...
        # Create DM via API
        # Include the placeholder user in participant_ids since the API uses it as current_user
        request_data = {
            "type": ChatType.DM,
            "participant_ids": [str(placeholder_user.id), str(alice.id)],
        }
        ...
        assert str(placeholder_user.id) in member_ids
    ...
    def test_complete_group_chat_flow(
        ...
        request_data = {
            "type": ChatType.GROUP,
            "participant_ids": [str(placeholder_user.id), str(bob.id), str(charlie.id)],
        ...
        # Verify members
        member_ids = {member["user_id"] for member in data["data"]["members"]}
        assert str(placeholder_user.id) in member_ids  # Creator
        assert str(bob.id) in member_ids
        assert str(charlie.id) in member_ids

        # Verify roles
        for member in data["data"]["members"]:
            if member["user_id"] == str(placeholder_user.id):
    ...
    def test_member_management_flow(
        ...
        request_data = {
            "type": ChatType.GROUP,
            "participant_ids": [str(placeholder_user.id), str(bob.id)],
            "topic": "Test Group",
        }
        ...
        add_response = client.post(f"/api/v1/chats/{chat_id}/members", json=add_request)

        # Should succeed since placeholder user is admin
        assert add_response.status_code == 201

        # Test removing member (will fail due to authentication)
        remove_response = client.delete(f"/api/v1/chats/{chat_id}/members/{bob.id}")
    ...
    def test_authorization_flow(
        ...
        details_response = client.get(f"/api/v1/chats/{chat_id}")
        assert details_response.status_code == 200  # OK

        # Test accessing chat members (should succeed since placeholder user is member)
        members_response = client.get(f"/api/v1/chats/{chat_id}/members")
        assert members_response.status_code == 200  # OK

    def test_error_handling_flow(self, client: TestClient, user_repo, clean_db):
        ...
        response = client.post("/api/v1/chats/", json=invalid_request)
        assert response.status_code == 422  # Validation error

        # Test non-existent chat access
        fake_chat_id = str(uuid4())
        response = client.get(f"/api/v1/chats/{fake_chat_id}")
    ...
    def test_data_consistency_flow(
        ...
        expected_dm_key = f"{min(str(placeholder_user.id), str(alice.id))}::{max(str(placeholder_user.id), str(alice.id))}"
